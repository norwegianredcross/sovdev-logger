...
export class EnvConfig {
  ...
  /**
   * Get JSON data from an environment variable, returning null if parsing fails
   * This is useful for fallback logic where you want to detect parsing failures
   */
  static getJSONOrNull<T = any>(
    envKey: string,
    options: ParseOptions = {}
  ): T | null {
    const opts = { ...this.DEFAULT_OPTIONS, ...options };
    const result = this.parseJSON<T | null>(envKey, null, opts);
